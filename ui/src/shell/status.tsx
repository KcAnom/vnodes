 ...
 *
 * `/ui/api/health` is fetched on mount and again when `?kb=` changes: doctor's
 * checks, the log tails and the effective config do not change on a timer, and
 * a page that re-ran doctor every ten seconds would be doing real work — port
 * probes, a manifest read — to redraw seven lines that almost never move. The
 * one thing that call *does* feed back into the loop is `ui.sidebar_refresh_s`,
 * which is how often `/status` is asked. That key has been in the config since
 * the first milestone with nothing reading it; this is the reader.
 *
